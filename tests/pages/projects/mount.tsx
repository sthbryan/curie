...
export function buttonLabelled(el: HTMLElement, label: string) {
  return el.querySelector<HTMLButtonElement>(`button[aria-label="${label}"]`);
}

export function projectFixture(name: string, overrides: Partial<Project> = {}): Project {
  return {
    id: name,
    name,
    path: `/code/${name}`,
...
export function summaryFixture(overrides: Partial<ProjectSummary> = {}): ProjectSummary {
  return { count: 2, agents: ["Codex"], missing: false, error: null, ...overrides };
}
