...
export function ResultsPanel({
  ...
  const columns = useMemo(
    (): ColumnDef<SkillSearchResult>[] => [
      {
        key: "name",
        header: t(lang.value, "find.colName"),
      ...
      {
        key: "source",
        header: t(lang.value, "find.colSource"),
        cellClassName: "min-w-0 flex flex-col gap-1",
        cell: (result) => (
          <>
      ...
      {
        key: "installs",
        header: t(lang.value, "find.colInstalls"),
      ...
      {
        key: "actions",
        header: t(lang.value, "find.colActions"),
        headerClassName: "text-right",
    ...
    [lang.value, installedPackages, installingPackage, installBusy, onInstall],
  );
    ...
    <Switch>
      <Case condition={showHint}>
        <motion.div {...fadeUp(0.08)} className="border-t border-border py-8">
          <p className="font-body text-sm text-fg-3">{t(lang.value, "find.hint")}</p>
        </motion.div>
      </Case>
      ...
      <Case condition={empty}>
        ...
        >
          <span className="font-body text-sm text-fg">{t(lang.value, "find.empty")}</span>
          <p className="font-body text-sm text-fg-3">{t(lang.value, "find.emptyHint")}</p>
        </motion.div>
      </Case>
