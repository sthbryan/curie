...
export function PlanRow({ icon: Icon, title, hint, path, chip, chipClassName = "" }: Props) {
  return (
    <div className="-mt-px flex items-start gap-5 border border-border bg-surface-tint px-5 py-4 first:mt-0">
      ...
      <div className="flex min-w-0 flex-col gap-1">
        ...
        <span className="font-body text-sm text-fg-3">{hint}</span>
        {path ? (
          <span className="truncate font-mono text-mono text-fg-4" title={path}>
            {path}
          </span>
        ) : null}
      </div>
